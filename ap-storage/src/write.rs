...
pub trait Write<'a> {
    ...
    async fn write_bytes(&'a self, offset: Offset, buf: &'a [u8]) -> Result<usize, Error>;
}
...
pub trait WriteExt<'a>: Write<'a> {
    ...
    async fn write_slice<T: Sized>(&'a self, offset: Offset, buf: &'a [T]) -> Result<usize, Error> {
        let x = unsafe {
            core::slice::from_raw_parts(buf.as_ptr() as *const u8, core::mem::size_of_val(buf))
        };
        let res = self.write_bytes(offset, x).await?;

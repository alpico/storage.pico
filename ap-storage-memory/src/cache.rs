...
impl<'a> MemoryCacheImpl<'a> {
    ...
    pub fn new(data: &'a mut [u8], parent: &'a dyn Read) -> Self {
        ...
        // we seperate the backing data here - the first is used for the pages, the remaining for the metadata
    ...
    pub fn read_mut(&mut self, ofs: Offset, buf: &mut [u8]) -> Result<usize, Error> {
        ...
        let maxn = core::cmp::min(maxn, n);
        buf[..maxn].copy_from_slice(&our[..maxn]);
        Ok(maxn)
    }
}

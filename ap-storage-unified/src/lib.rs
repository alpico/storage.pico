...
use ap_storage::{FileSystem, Read, file::File};
...
impl<'a> File for UnifiedFile<'a> {
    type Dir<'c> = UnifiedDir<'c> where Self: 'c;
...
pub enum UnifiedDir<'a> {
    Ext4(<<Ext4Fs<'a> as FileSystem<'a>>::FileType as File>::Dir<'a>),
    Vfat(<<VFatFS<'a> as FileSystem<'a>>::FileType as File>::Dir<'a>),
    Json(<<JsonFS as FileSystem<'a>>::FileType as File>::Dir<'a>),
}

use ap_storage::directory::{Iterator, Item};

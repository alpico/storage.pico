...
use al_crunch_pool::{PoolOptions, Sender, execute};
...
fn visit(sender: &Sender<WorkerState>, nr: u64, worker: &mut WorkerState) {
    ...
    let Some(mut iter) = dir.dir(worker.leaf_optimization) else { return };
    ...
    while let Ok(entry) = iter.next(&mut buf)  {
        ...
        let Ok(child) = dir.open(entry.inode()) else { continue };
...
fn main() -> Result<(), Error> {
    ...
    let fs = Ext4Fs::mount(unsafe { std::mem::transmute(x) })?;
    ...
    let make_state = |_| {
        WorkerState {
            fs: fs.clone(),
            leaf_optimization: args.leaf_optimization,
            size: 0,
            count: 0,
        }};


    let state = execute(config,
                        make_state,
                        |sender| {

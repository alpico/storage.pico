...
fn visit(dir: &File<'_>, leaf_optimization: bool) -> Result<(usize, u64), Error> {
    let Some(mut iter) = dir.dir(leaf_optimization) else { return Ok((0, 0)) };
    let mut buf = [0u8;255];
    let mut count = 0;
    let mut size = 0;
    ...
    while let Ok(entry) = iter.next(&mut buf)  {
        ...
        size += child.size();
        if entry.is_dir() {
            let (x,y) = visit(&child, leaf_optimization)?;
            count += x;
            size += y;
        }
    };
...
fn main() -> Result<(), Error> {
    ...
    let dir = fs.root()?;
    let (count, size) = visit(&dir, args.leaf_optimization)?;

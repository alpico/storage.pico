...
impl<'a> Ext4Blocks<'a> {
    ...
    pub fn search(&self, mut _block: u64) -> Result<(u64, u64), Error> {
        use ap_storage::ReadExt;
            ...
            _block -= 12;
            let log_numbers_per_block = self.0.block_size.ilog2() as usize - 2;
            ...
            while level > 0 && res != 0 {
                ...
                if level != 1 || (index + Self::MAX_MERGED as u64) >> log_numbers_per_block > 0 {
                    res = self.0
                        .fs
                        .disk
                        .read_object(res as u64 * self.0.block_size + index * 4)?;
                } else {
                    let blocks: [u32; Ext4Blocks::MAX_MERGED] = self.0
                        .fs
                        .disk
                        .read_object(res as u64 * self.0.block_size + index * 4)?;
                    res = blocks[0];
